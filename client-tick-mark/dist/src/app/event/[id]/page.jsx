...
const Info = ({ ticketDetails, ticketIdViewFn }) => {
    ...
    return (<div className="card bg-base-100 image-full w-96 shadow-sm rounded-sm">
          <figure>
            <img className="w-full h-full" src="/logo-files/swiftpass-logo-only-svg.svg" alt="Swiftpass logo"/>
          </figure>
          <div className="card-body">
            ...
            <button onClick={e => handleTicketPurchase(e, true)} className="px-4 py-2 bg-emerald-600 font-semibold rounded-sm hover:cursor-pointer text-white">
                Purchase Ticket
            </button>
          </div>
        </div>);
...
const EventInfo = () => {
    ...
    if (isLoading)
        return <div className="font-semibold font-gray-600 flex flex-row items-center justify-center">
    ...
    if (error)
        return <p className="font-semibold font-gray-600 text-center">Error loading data</p>;
    return (<div className="p-2">
            <div className="grid grid-col items-center justify-center mt-8">
                <div className="py-3 flex flex-col justify-center items-center">
                    <h1 className="text-emrald-800 text-2xl font-bold text-center">{currentEvent === null || currentEvent === void 0 ? void 0 : currentEvent.title}</h1>
                    <div>
                        <p className="text-center flex flex-row items-center text-emerald-500">
                            ...
                            {currentEvent === null || currentEvent === void 0 ? void 0 : currentEvent.venue}
                        </p>
                    </div>
                    <h3 className="text-gray-600 font-bold text-lg">tickets</h3>
                    {!!session && (<div>
                            {((_b = (_a = session === null || session === void 0 ? void 0 : session.user) === null || _a === void 0 ? void 0 : _a.user) === null || _b === void 0 ? void 0 : _b.organization_id) === (currentEvent === null || currentEvent === void 0 ? void 0 : currentEvent.owner_id) && (<button onClick={() => router.push(`/qrscan/${currentEvent === null || currentEvent === void 0 ? void 0 : currentEvent.owner_id}/event/${currentEvent === null || currentEvent === void 0 ? void 0 : currentEvent.event_id}`)} className="bg-emerald-700 text-emerald-50 p-1 hover:cursor-pointer rounded-xs text-sm">
                                    Scan ticket
                                </button>)}
                        </div>)}
                </div>
                {data ? (<div className="flex flex-col gap-y-4 justify-center items-center">
