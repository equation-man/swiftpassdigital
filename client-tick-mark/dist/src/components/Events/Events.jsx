...
const Events = ({ owner }) => {
    ...
    if (error)
        return <p className="font-semibold font-gray-600 text-center p-4">Error fetching data</p>;
    if (isLoading)
        return <div className="flex flex-col items-center justify-center p-4 text-emerald-700">
    ...
    return (<div className="carousel carousel-center rounded-box w-full space-x-4 px-2 py-4">
            {data ? (<>
                    {data.filter(itm => {
                ...
                return sessionFilter !== itm.owner_id && eventEnd >= now;
                    ...
                    <p className="font-semibold text-emerald-800 text-center">Failed to fetch data. Refresh</p>
                </div>)}
        </div>);
};
