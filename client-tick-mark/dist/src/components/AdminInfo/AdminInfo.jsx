...
const AddUserAccess = ({ o_id }) => {
    ...
    return (<div className="w-90 p-1 shadow-xl">
...
const UsersAccess = ({ user_access }) => {
    ...
    const mutation = useMutation({
        ...
        mutationFn: ({ organization_id, access_code_id }) => deleteUserAccessFn(organization_id, access_code_id),
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ["accessors"] });
            toast.dismiss();
    ...
    const handleDeleteAccess = async (event) => {
        event.preventDefault();
    ...
    return (<div className="w-90 p-2 shadow-xl flex flex-row items-center gap-x-4 justify-between">
            <div>
            ...
            <div>
                <button onClick={handleDeleteAccess} className="text-rose-50 bg-rose-500 p-1 btn-block hover:cursor-pointer">
                    Revoke
                </button>
            </div>
        </div>);
            ...
            <div>
                ...
                <div>
                    <div className="w-90 border border-emerald-500 border-2px rounded-sm py-2 my-3 flex flex-col items-center justify-center hover:cursor-pointer">
                        ...
                        {data && data.wallet_email !== 'none@notset.com' ? (<div className="w-full p-2">
                                <div className="">
                            ...
                            </div>) : (<button onClick={(e) => handleWalletCreation(e, true)} className="bg-emerald-700 text-white px-2 py-1 rounded-sm text-sm flex flex-row items-center gap-x-1 hover:cursor-pointer">
                                Add wallet
                                <svg xmlns="http://www.w3.org/2000/svg" width={24} height={24} viewBox="0 0 24 24">
                                    <path fill="currentColor" d="M19 12a1 1 0 0 1 1-1h.01a1 1 0 1 1 0 2H20a1 1 0 0 1-1-1"></path>
                                    <path fill="currentColor" fillRule="evenodd" d="M18.6 4H3.4A2.4 2.4 0 0 0 1 6.4v11.2A2.4 2.4 0 0 0 3.4 20h15.2a2.4 2.4 0 0 0 2.4-2.4V16h.4a2.6 2.6 0 0 0 2.6-2.6v-2.8A2.6 2.6 0 0 0 21.4 8H21V6.4A2.4 2.4 0 0 0 18.6 4m-2 6a.6.6 0 0 0-.6.6v2.8a.6.6 0 0 0 .6.6h4.8a.6.6 0 0 0 .6-.6v-2.8a.6.6 0 0 0-.6-.6z" clipRule="evenodd"></path>
                    ...
                    <div>
                        {createWallet && (<div className="max-w-xs md:max-w-sm p-1 shadow-xl border border-emerald-500 border-2px">
                                ...
                                <div className="flex flex-row gap-x-2 my-2">
                                    <button onClick={(e) => handleWalletCreation(e, false)} className="hover:cursor-pointer bg-emerald-600 text-white btn-block p-1">
                                        Cancel
                                    </button>
                                    <button type="submit" form="createWalletForm" className="hover:cursor-pointer bg-emerald-800 text-white btn-block p-1">
                                        Save Changes
                                    </button>
                                </div>
                            </div>)}
            ...
            <div className="pt-5">
                <div>
                    <div className="flex flex-row gap-x-2">
                        ...
                        {addAccess ? (<button onClick={() => setAddAccess(false)} className="bg-white text-emerald-700 px-1 text-xs rounded-sm flex flex-row gap-x-1 items-center hover:cursor-pointer border border-emerald-700">
                                Close form
                            ...
                            </button>) : (<button onClick={() => setAddAccess(true)} className="bg-white text-emerald-700 px-1 text-xs rounded-sm flex flex-row gap-x-1 items-center hover:cursor-pointer border border-emerald-700">
                ...
                <div>
                    ...
                    {accessList.isSuccess ? (<div>
                            {accessList.data.map((usr) => <UsersAccess key={usr.access_code_id} user_access={usr}/>)}
                        </div>) : (<p className="font-semibold text-gray-800">No external access granted</p>)}
                </div>
            </div>
        </div>);
};
