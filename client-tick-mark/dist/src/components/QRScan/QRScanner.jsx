...
export default function QRScanner() {
    ...
    async function startScanner() {
        ...
        try {
            await html5Qr.start({ facingMode: "environment" }, config, async (decodedText, decodedResult) => {
                ...
                setLastResult(Object.assign({}, data));
                ...
                try {
                    ...
                    setLastResult(Object.assign({}, res));
                }
                catch (e) {
                    setLastResult({
                        ok: false,
    ...
    return (<div className="flex flex-col items-center space-b-6 space-t-3 pb-10 pt-5">
            <h1 className="text-2xl font-semibold">Scan Ticket QR code</h1>
            <div id={readerId} className="rounded-sm border border-gray-300 shadow-inner w-[320px] h-[320px] bg-gray-50 flex items-center justify-center">
                {!scanning && (<span className="text-gray-400 text-sm">Camera feed will appear here</span>)}
            </div>
            <div className="flex gap-4 py-8 mt-8">
                <button onClick={startScanner} disabled={scanning} className={`px-5 py-2 rounded-sm text-white font-medium transition hover:bg-emerald-800 ${scanning ? "bg-gray-400 cursor-not-allowed" : "bg-emerald-700 hover:bg-indigo-700"}`}>
                    {scanning ? "Scanning..." : "Start scanning"}
                </button>
                <button onClick={stopScanner} disabled={!scanning} className={`px-5 py-2 rounded-sm font-medium border transition ${scanning ? "border-rose-500 text-rose-600 hover:bg-rose-50" : "border-teal-500 text-teal-800 cursr-not-allowed"}`}>
                    Stop scanning
                </button>
            </div>
            {lastResult && (<div className={`rounded-sm p-2 text-center w-[320px] shadow-lg ${mutation.isSuccess | mutation.isPending ? "bg-gradient-to-r from-emerald-100 via-emerald-50 via-white-100 to-green-100 text-emerald-800" : "bg-rose-50 border-rose-400 text-rose-700"}`}>
                    {mutation.isPending && (<div className="flex flex-col items-center justify-center text-emerald-700">
                    ...
                    {mutation.isSuccess && (<div className="flex flex-col items-center justify-center">
                    ...
                    {mutation.isError && (<div className="flex flex-col items-center justify-center">
