...
pub fn events_routes(cfg: &mut web::ServiceConfig) {
    cfg.service(
        ...
        .route("/ticket/mpesa/purchase/{ticket_id}", web::post().to(mpesa_order_and_callback)) //rm-mpesa_order
        ...
        .route("/ticket/qrverify/{org_id}/{event_id}", web::patch().to(qr_verify))
        .route("/ticket/verify/paystack/order/{ticket_id}", web::get().to(verify_paystack_order))
        .route("/report/{event_id}", web::get().to(events_report))
    );
}
