...
DROP FUNCTION IF EXISTS ticket_market.search_events(
    search_query TEXT,
    title TEXT,
    venue TEXT,
    event_tag TEXT,
...
CREATE OR REPLACE FUNCTION ticket_market.search_events(
    ...
    _event_tag TEXT DEFAULT NULL,
    _min_date TIMESTAMPTZ DEFAULT NULL,
    _max_date TIMESTAMPTZ DEFAULT NULL,
...
BEGIN
    ...
    WHERE e.search_vector @@ tsquery_var
        AND finish_time > NOW()
        AND (_title IS NULL OR e.title ILIKE '%' || _title || '%')
        AND (_venue IS NULL OR e.venue ILIKE '%' || _venue || '%')
        AND (_event_tag IS NULL OR e.event_tag = _event_tag)
        AND (_min_date IS NULL OR e.start_time >= _min_date)
        AND (_max_date IS NULL OR e.start_time <= _max_date)
    INTO total;
